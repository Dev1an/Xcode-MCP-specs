...
Generated from JSON containing 47 tools.

## Table of Contents

- [1. AddEntitlement](#1-addentitlement)
- [2. AddInfoPlist](#2-addinfoplist)
- [3. Build Project](#3-build-project)
- [4. End Device Interaction Session](#4-end-device-interaction-session)
- [5. Prepare for device interaction: install and start application on a device](#5-prepare-for-device-interaction-install-and-start-application-on-a-device)
- [6. Start Device Interaction Session](#6-start-device-interaction-session)
- [7. Perform single device interaction and fetch device state](#7-perform-single-device-interaction-and-fetch-device-state)
- [8. DocumentationSearch](#8-documentationsearch)
- [9. Get Build Log](#9-get-build-log)
- [10. GetConsoleOutput](#10-getconsoleoutput)
- [11. Get Crash Issue Logs](#11-get-crash-issue-logs)
- [12. Get Field Performance Issue Logs](#12-get-field-performance-issue-logs)
- [13. GetFileCompilerFlags](#13-getfilecompilerflags)
- [14. GetTargetBuildSettings](#14-gettargetbuildsettings)
- [15. Get Test List](#15-get-test-list)
- [16. Get Top Crash Issues](#16-get-top-crash-issues)
- [17. Get Top Field Performance Issues](#17-get-top-field-performance-issues)
- [18. Invoke a Debugger Command](#18-invoke-a-debugger-command)
- [19. Prepare For Localization](#19-prepare-for-localization)
- [20. RenderPreview](#20-renderpreview)
- [21. Run All Tests](#21-run-all-tests)
- [22. RunCodeSnippet](#22-runcodesnippet)
- [23. Run Project](#23-run-project)
- [24. Run Some Tests](#24-run-some-tests)
- [25. Stop Project](#25-stop-project)
- [26. Get String Context](#26-get-string-context)
- [27. Insert Translation](#27-insert-translation)
- [28. Read String Catalog](#28-read-string-catalog)
- [29. UpdateFileCompilerFlags](#29-updatefilecompilerflags)
- [30. UpdateTargetBuildSetting](#30-updatetargetbuildsetting)
- [31. Get Current File](#31-get-current-file)
- [32. Search Files](#32-search-files)
- [33. Search Content](#33-search-content)
- [34. List Workspace Issues](#34-list-workspace-issues)
- [35. List Run Destinations](#35-list-run-destinations)
- [36. List Schemes](#36-list-schemes)
- [37. List Windows](#37-list-windows)
- [38. List Directory Contents](#38-list-directory-contents)
- [39. Create Directory](#39-create-directory)
- [40. Move File](#40-move-file)
- [41. Read](#41-read)
- [42. XcodeRefreshCodeIssuesInFile](#42-xcoderefreshcodeissuesinfile)
- [43. Remove File](#43-remove-file)
- [44. Switch Run Destination](#44-switch-run-destination)
- [45. Switch Scheme](#45-switch-scheme)
- [46. Edit File](#46-edit-file)
- [47. Write File](#47-write-file)
...
| ---------------------------- | ------------------------------------------------------------ | -------: | ------------------------------------------------------------ |
| `entitlementDictionaryItems` | string                                                       |       No | A JSON-encoded string representing a dictionary. Required when entitlementValueType is 'dictionary'. Example: '{"key":"value"}'. |
| `entitlementKey`             | string                                                       |      Yes | The entitlement key you want to add.                         |
| `entitlementValue`           | string                                                       |       No | The entitlement value as a string. Required when entitlementValueType is 'bool', 'string', or 'int'. For bool, pass 'true' or 'false'. For int, pass the number as a string. Not used for stringArray or dictionary types. |
| `entitlementValueItems`      | array<string>                                                |       No | Array of string values. Required when entitlementValueType is 'stringArray'. |
| `entitlementValueType`       | `bool` \| `string` \| `int` \| `stringArray` \| `dictionary` |      Yes | The type of the entitlement value. Most entitlements are 'bool' (pass entitlementValue 'true'). Some are 'string' (pass the value in entitlementValue). A few are 'stringArray' (pass values in entitlementValueItems instead). Use 'dictionary' for entitlements whose value is a plist dictionary (pass JSON in entitlementDictionaryItems). |
| `tabIdentifier`              | string                                                       |      Yes | The workspace tab identifier.                                |
| `targetName`                 | string                                                       |      Yes | The name of the Xcode target to add the entitlement to. Use another tool (e.g. GetTargetBuildSettings) to discover target names if they are not already known. |
...
| Field              | Type    | Required | Description                                                  |
| ------------------ | ------- | -------: | ------------------------------------------------------------ |
| `errorDescription` | string  |       No | An optional description of the error when trying to add the entitlement. This string will be available if result is false. |
| `result`           | boolean |      Yes | A boolean to indicate if adding the entitlement was successful or not. |
...
| Field                      | Type                                                         | Required | Description                                                  |
| -------------------------- | ------------------------------------------------------------ | -------: | ------------------------------------------------------------ |
| `infoPlistDictionaryItems` | string                                                       |       No | A JSON-encoded string representing an array of dictionaries. Required when infoPlistValueType is 'dictionaryArray'. Example: '[{"NSAllowsArbitraryLoads": true}]' |
| `infoPlistKey`             | string                                                       |      Yes | The Info.plist key to add or change                          |
| `infoPlistValue`           | string                                                       |       No | The value as a string. Required when infoPlistValueType is 'bool', 'string', or 'int'. For bool, pass 'true' or 'false'. For int, pass the number as a string. Not used for stringArray or dictionaryArray types. |
| `infoPlistValueItems`      | array<string>                                                |       No | Array of string values. Required when infoPlistValueType is 'stringArray'. |
| `infoPlistValueType`       | `bool` \| `string` \| `int` \| `stringArray` \| `dictionaryArray` |      Yes | The type of the Info.plist value. Common types are 'string' (for privacy descriptions, display names) and 'bool' (for boolean flags). Use 'stringArray' for lists and 'dictionaryArray' for complex structures like App Transport Security settings. |
| `tabIdentifier`            | string                                                       |      Yes | The workspace tab identifier                                 |
| `targetName`               | string                                                       |      Yes | The name of the Xcode target whose Info.plist key should be added or updated. Use another tool (e.g. GetTargetBuildSettings) to discover target names if they are not already known. |
...
| Field              | Type    | Required | Description                                                  |
| ------------------ | ------- | -------: | ------------------------------------------------------------ |
| `errorDescription` | string  |       No | An optional description of the error when trying to add the info.plist. This string will be available if result is false. |
| `result`           | boolean |      Yes | Whether or not adding/updating the info.plist was successful. |
...
## 4. End Device Interaction Session
...
## 5. Prepare for device interaction: install and start application on a device
...
## 6. Start Device Interaction Session
...
## 7. Perform single device interaction and fetch device state
...
## 12. Get Field Performance Issue Logs
...
| `projectPath`   | string |       No | The project-organization path of the .xcodeproj that owns the target (e.g., 'MyApp/MyApp.xcodeproj'). Only needed when the same target name exists in multiple projects in the workspace; omit otherwise. An absolute or workspace-relative filesystem path is also accepted as a fallback. |
| `tabIdentifier` | string |      Yes | The workspace tab identifier.                                |
...
| --------------- | ------ | -------: | ----------------------------------------- |
| `tabIdentifier` | string |      Yes | The workspace tab identifier.             |
| `targetName`    | string |      Yes | The name of a given Xcode project target. |
...
| Field           | Type          | Required | Description                                                  |
| --------------- | ------------- | -------: | ------------------------------------------------------------ |
| `buildSettings` | array<object> |      Yes | An array of build settings for the named target.             |
| `targetName`    | string        |      Yes | The real name of a Xcode project target, also known as a blueprint. |
...
## 17. Get Top Field Performance Issues
...
| `relevantPluralCases`                 | array<string> |      Yes | If the string contains numerical format specifiers, this lists the relevant plural cases for the target locale (e.g., ['zero', 'one', 'other']). When empty, this string doesn't require any pluralization. |
...
## 30. UpdateTargetBuildSetting

**Name:** `UpdateTargetBuildSetting`


### Description

Updates, appends or deletes the named build setting for the specified Xcode project target.
Always use this tool to update/change the build settings for a target.
If you need to get build settings, always use the GetTargetBuildSettings tool.

IMPORTANT: Do NOT try to modify the project.pbxproj file directly.
IMPORTANT: Do NOT try to read the project.pbxproj file directly; use the GetTargetBuildSettings tool instead."
...
| Field               | Type    | Required | Description                                                  |
| ------------------- | ------- | -------: | ------------------------------------------------------------ |
| `appendValue`       | boolean |       No | Append the value instead of replacing the current value.     |
| `buildSettingName`  | string  |      Yes | The build setting name to update or add.                     |
| `buildSettingValue` | string  |       No | The value to be added to build settings.<br>Do not convert "NO" values to "false".<br>To delete the build setting instead, just skip this property. |
| `tabIdentifier`     | string  |      Yes | The workspace tab identifier.                                |
| `targetName`        | string  |      Yes | The name of a given Xcode project target.                    |
...
## 42. XcodeRefreshCodeIssuesInFile
